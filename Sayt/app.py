...
server_address = ("cofesubd-service.default.svc.cluster.local", 6379)
statistic_address = ("statsservice-service.default.svc.cluster.local", 5003)
    ...
    if request.method == 'POST':
        ...
        if len(original_link) < 110000:
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    ...
                    s.sendall(f"gdf8g90fd --file shrtlnks.data --query 'HSET shrtlnks {short_link} {original_link}'".encode())
            ...
            generated_link = f"http://37.193.53.6:32191/{short_link}"
...
def redirect_to_original(short_link):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        try:
            s.connect(server_address)
            s.sendall(f"dgdf7g8dfg --file shrtlnks.data --query 'HGET shrtlnks {short_link}'".encode())
            data = s.recv(1024)
